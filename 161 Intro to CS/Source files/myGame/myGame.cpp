...
const char* DICTIONARY_NAME = "dict2.txt";
const char* SAVED_NAME = "last.txt";
const char* RECORDS_NAME = "highscores.txt";
...
int main()
{
    ...
    ifstream dictionary, savedGame, recordHolder;
    ofstream lastGame, records;
    ...
    savedGame.open(SAVED_NAME);

    if(savedGame >> total)
    {
        //figure out if they want to play another game
        cout << "Do you want to continue your last saved game?" << endl;
        response = ResponseCheck('Y', 'N', '\0');
        cout << space;

        if(response != 'N')
        {
            savedGame >> totalGames;
        }
        else
        {
            total = 0;
    ...
    do
    {
        ...
        int availNum[length]; //stores numbers that represent letters
       ...
       /* while(secret.empty() || isNotLetters)
            ...
            if (total > 4 || points > 0)
            {
                ...
                if (buy)
                {
                    ...
                    if(guess != secret)
                    {
                        ...
                        if (inWord)
                        {
                            cout << "\nYour guess was in the secret word!" << endl;
                        }
                        else
                        {
        ...
        else
        {
            ...
            {
                char missed = '\0'; //missed letter in secret
        ...
        if(response == 'N' && !endGame)
        {
            //figure out if they want to play another game
            cout << "\nDo you want to save your game?" << endl;
            response = ResponseCheck('Y', 'N', '\0');
            if(response != 'N')
            {
                lastGame.open(SAVED_NAME);
                lastGame << total << '\0';
                lastGame << totalGames << endl;
                lastGame.close();
                response = 'N';
            }
            else
            {
                endGame = true;
                lastGame.open(SAVED_NAME, ofstream::out | ofstream::trunc);
                lastGame.close();
            }
        }
        ...
        if(endGame)
        {
            recordHolder.open(RECORDS_NAME);
            int pastTotal = -1;
            int pastTotals[10];
            int place;
            int totalTemp = total;
            int i = 0;
            bool notUsed = true;
            while(recordHolder >> pastTotal && i < 5)
            {
                if(pastTotal < totalTemp)
                {
                    pastTotals[i] = total;
                    place = i + 1;
                    notUsed = false;
            ...
            if (!notUsed || i < 4)
            {
                ...
                cout << "You got a new " << (place) << ((place) == 1? "st" : "")
                     << ((place) == 2? "nd" : "") << ((place) == 3? "rd" : "")
                     << (((place) == 4 || (place) == 5)? "th" : "")
                     << " place record with " << total << " point" << (total == 1? "." : "s.") <<endl;
            }
            ...
            records.open(RECORDS_NAME);
            for (int j = 0; j < 5; j++)
            {
                records << pastTotals[j] << endl;
            }
            records.close();
            endGame = false;
        }
