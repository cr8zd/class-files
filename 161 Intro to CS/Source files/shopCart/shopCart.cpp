...
void listCart(const Item [], const int);

//prints total for all items in the cart
...
int main()
{
    ...
    Item cart [MAX_SIZE]; //storage for the cart
    char response; //user input
    int cartNum = 0; //number of times an item is added
    ...
    do
    {
        ...
        {
            addItem(cart, cartNum);
            cartNum++;
        }
        else if (response == '2') //list cart
        {
            listCart(cart, cartNum);
        }
        else if (response == '3') //print total
        {
            printTotal(cart, cartNum);
        }
...
******************************************************************************/
void printTotal(const Item cart[], const int cartNum)
{
...
******************************************************************************/
void listCart(const Item cart[], const int cartNum)
{
    ...
    for(int i = 0; i < cartNum; i++)
    {
        ...
        cout << right << setw(10) << cart[i].price;
        cout << setw(12) << cart[i].quantity << endl;
    }
...
******************************************************************************/
void addItem(Item *cart, int cartNum)
{
    ...
    object.price = doubleCheck(0, INT_MAX, 2);

    cout << "\nHow many of this item do you want?" << endl;
    object.quantity = intCheck(0,INT_MAX);
