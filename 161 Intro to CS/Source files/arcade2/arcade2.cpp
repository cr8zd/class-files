...
int main()
{
	...
	int remainCoup; // coupons left over after
	int bars = 0; // how many candy bars you can get
	int gum = 0; // how many gum balls you can get
	...
	while (userInput < 0 || abs(remainder) > .0000000001)
	{
	    ...
	    while (abs(remainder) > .0000000001)
		{
		    cout << "You cannot turn in partial coupons, and you gave:" << endl;
		    cout << userInput << ", a partial number for a coupon."
	...
	while (remainCoup >= BAR_RATE)
	{
		remainCoup -= BAR_RATE;
		bars ++;
	}

	// find the number of gum balls
	while (remainCoup >= GUM_RATE)
	{
		remainCoup -= GUM_RATE;
		gum ++;
	}

	//Print how many candy bars and gum balls
