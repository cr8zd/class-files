...
int main()
{
    ...
    do
    {
      ...
      /*  while(!available.empty())
        ...
        {
            int number = rand() % available.length();
            char letter = available[number];
            availNum[i] = letter;
            available.erase(available.begin() + number);
        }
        ...
        while (round > 0 && guess.compare(secret) != 0)
        {
                ...
                else if (response == '2')
                {
                    ...
                    do
                    {
                        ...
                        for (int i = letPos; i < length - (maxround -
                                                       (round - buyNum)); i++)//while (actor.find(letter) == string::npos)
                        {
                            if (availNum[i] == letter)
                                there = true;
                        }
            ...
            if (round > 0 && guess.compare(secret) != 0)
            {
                                              ...
                                              (round - buyNum)); i++)
                {
                    ...
                    number[i] = (int)availNum - 96

                }
                    cout << left << setw(3) << ((int)availNum[i] - 96)<< " ";
        ...
        if (guess == secret)
        {
            cout << "\nCongratulations, you found the secret word!"
                 << "\nYou get " << available.length() << " extra point"
                 << (available.length() == 1? "" : "s") << " remaining"
                 << endl;
            points += available.length();
        }
