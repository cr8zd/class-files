...
int main()
{
    ...
    do
    {
        char *availNum = new char[MAX_CHAR_NUM*length];
        actor.clear();
        past.clear();
        missing.clear();
        secret.clear();
        ...
        {
            string numStr = convert(i);
            char *currentNum = new char[3];
            for (int j = 0; j < numStr.length(); j++)
            {
                currentNum[j] = numStr[j];
            }
            for (int j = 0; j < MAX_CHAR_NUM; j++)
            {
                if (j > strlen(currentNum))
                {
                    availNum [i * MAX_CHAR_NUM + j] = '\0';
                }
                else
                {
                    availNum [i * MAX_CHAR_NUM + j] = currentNum[j];
                }
                cout << availNum [i * MAX_CHAR_NUM + j];
            }
            delete[] currentNum
        }
/*
