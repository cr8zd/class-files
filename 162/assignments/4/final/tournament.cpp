...
int main()
{
    ...
    do
    {
        ...
        int count = 0,
            team1 = 0,
            team2 = 0;
        std::map<int, Creature*>::reverse_iterator rit;
	for(rit = losers.rbegin(); rit != losers.rend(); ++rit)
        {
            if(count <  3)
                std::cout << rit->second->getName() << ": "
                          << rit->first << "pts for team "
                          << rit->second->getTeam() << std::endl;
            if(rit->second->getTeam() == 1)
                team1 += rit->first;
            else if(rit->second->getTeam() == 2)
                team2 += rit->first;
            count++;
        }
