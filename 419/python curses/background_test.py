...
class Background():
        ...
        def drawStars(self, x, y):
                self.backWin.addstr(y, x, "  .       *             .   .")#36 across biggest
        ...
        def drawAsteroidS(self, x, y):
                self.backWin.addstr(y, x,   "   ___" , curses.color_pair(4))
                self.backWin.addstr(y+1, x, "__/ O \\__ " , curses.color_pair(4))
                self.backWin.addstr(y+2, x, "\\  o     \\" , curses.color_pair(4))
                self.backWin.addstr(y+3, x, " |__   o _/" , curses.color_pair(4))
                self.backWin.addstr(y+4, x, "   \\___/" , curses.color_pair(4))


        def drawAsteroidM(self, x, y):
                self.backWin.addstr(y, x,   "        ___________", curses.color_pair(5))
                self.backWin.addstr(y+1, x, "     __/ O       | \\__ ", curses.color_pair(5))
                self.backWin.addstr(y+2, x, "    /   o        \\_/  \\", curses.color_pair(5))
                self.backWin.addstr(y+3, x, "   |         /\\        |", curses.color_pair(5))
                self.backWin.addstr(y+4, x, "    \___     \\/____O__/", curses.color_pair(5))
                self.backWin.addstr(y+5, x, "        \\o___/", curses.color_pair(5))

        def drawAsteroidL(self, x, y):
                self.backWin.addstr(y+1, x,    "             __________", curses.color_pair(6))
                self.backWin.addstr(y+2, x,  "          __/ O        \\___ ", curses.color_pair(6))
                self.backWin.addstr(y+3, x,  "         / o               \\_", curses.color_pair(6))
                self.backWin.addstr(y+4, x,  "         |       /\          \\", curses.color_pair(6))
                self.backWin.addstr(y+5, x,  "        /        \/       ___/", curses.color_pair(6))
                self.backWin.addstr(y+6, x,  "       /                 /", curses.color_pair(6))
                self.backWin.addstr(y+7, x,  "      |     __          /", curses.color_pair(6))
                self.backWin.addstr(y+8, x,  "      |    /  \\_    O   \\____ ", curses.color_pair(6))
                self.backWin.addstr(y+9, x,  "      |   /     |            \\", curses.color_pair(6))
                self.backWin.addstr(y+10, x,  "       \\__\\___  |             |", curses.color_pair(6))
                self.backWin.addstr(y+11, x, "          \___\\/       ___O__/", curses.color_pair(6))
                self.backWin.addstr(y+12, x, "              \\_______/", curses.color_pair(6))
        

        def drawCloud3(self, x, y):
                self.backWin.addstr(y, x,   "     ~~~~~")
                self.backWin.addstr(y+1, x, "  ~~~    ~~~~")
                self.backWin.addstr(y+2, x, "~~         ~~")
        ...
        def drawClouds(self):
                self.drawCloud3(1,0)
                self.drawCloud1(60,0)
                self.drawCloud2(15,9)
                self.drawCloud1(7,18)
...
class GameLoop:
        ...
        def configureCursesSettings(self):
                if(curses.COLORS == 256):
                        curses.init_pair(4, 245, curses.COLOR_BLACK)
                        curses.init_pair(5, 240, curses.COLOR_BLACK)
                        curses.init_pair(6, 247, curses.COLOR_BLACK)
                else:
